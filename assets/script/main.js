    ...
    const newArr = []
        ...
        newArr.forEach((coffee) => {
            ...
            increaseBtn.forEach((btn) =>{
                btn.addEventListener('click', function(){
                    console.log(this);
                    const coffeeId = this.parentElement.parentElement.querySelectorAll('.coffeeId').textContent
                    console.log(this.parentElement.parentElement.querySelectorAll('.coffeeId'));
                    const dataFromLocal = JSON.parse(localStorage.getItem("cart"))
                    const findedCoffee = dataFromLocal.find((x) => x.id === coffeeId)

                    if(findedCoffee){
                        findedCoffee.count++
                        localStorage.setItem('cart', JSON.stringify(dataFromLocal))
                        this.parentElement.previousElementSibling.previousElementSibling.textContent = findedCoffee.count
                    }
                })
            })
